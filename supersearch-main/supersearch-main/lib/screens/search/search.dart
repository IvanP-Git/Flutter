...
class _SearchState extends State<Search> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: const Text('Search Users'), backgroundColor: Colors.blueAccent,
        ),
  ...
  Future<List<String>> _searchUsers(String name) async {
    ...
    final result = await Supabase.instance.client // Fer servir un tsvector i implementar una cerca per text.
        .from('names')
        .select('fname, lname')
        .textSearch('fts', "$name:*")
        .limit(100);
